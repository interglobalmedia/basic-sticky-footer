...
<body>
    ...
    <footer class="site-credits">Footer</footer>
</body>
```
...
***Even though*** there is ***very little*** `content` on the ***page***, the `footer` ***still sticks*** to the ***bottom*** of the `page`.
</section>
...
***Next***, we ***add*** a `body selector rule set` ***containing*** the ***following*** `CSS property declarations` ***inside***:
...
In our `flex` ***shorthand*** `property declaration`, the `flex-shrink` ***property value*** is `0`. which ***means*** that the ***targeted*** `flex-item` (`main element`) does ***not shrink***. it will ***retain*** the `width` it ***needs***, and ***not wrap*** its `content` (which is ***what*** it ***does*** when it ***shrinks***). Its ***siblings*** (if there were any in our case) will ***shrink*** to ***give space*** to the `target element` (***here*** it is the `main element`).

***Because*** the `target element` will ***not wrap*** its ***content***, there is ***also*** a ***chance*** for the `flexbox container`'s (here it is the `body element`) `content` to ***overflow***.
